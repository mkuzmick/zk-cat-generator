...
export default function CatImage() {
  ...
  const getCatBackstory = async () => {
    if (!imageData || !suggestedName) {
      console.error('Cannot get backstory: missing image data or name', {
    ...
    console.log('Requesting backstory for', suggestedName);
    setIsLoadingDetails(true);
    try {
      // Call our API to get backstory and character details
      const payload = {
        imageData,
        action: 'getBackstory',
        name: suggestedName
      ...
      console.log('Received response from cat details API');
      const data = await response.json();
      ...
      if (!data.backstory || !data.personalityType || !data.dndAttributes) {
        console.error('Missing expected data in response:', data);
        throw new Error('Incomplete data returned from server');
      }
      
      console.log('Setting cat details with backstory length:', data.backstory.length);
      console.log('Personality type:', data.personalityType);
      console.log('D&D attributes:', data.dndAttributes);
    ...
    <main className="flex min-h-screen flex-col items-center justify-between p-8" style={{ backgroundColor: "#f9f3e5", color: "#4a3520" }}>
      <div className="relative flex flex-col place-items-center gap-8">
        ...
        <button
          ...
          className="px-6 py-3 mt-6 font-bold rounded-lg transition-colors"
          style={{ backgroundColor: '#8c6d4f', color: '#fff9ec', borderColor: '#4a3520' }}
        >
          Generate New Cat
        </button>
        
        <Link href="/" className="mt-4 hover:underline" style={{ color: '#8c6d4f' }}>
          Back to Home
        </Link>
      </div>
    </main>
